...
export function MonthGrid({ currentDate, events, onSelectDay }: {
  ...
  return <div className="calendar-month-shell w-full min-w-0 overflow-x-auto rounded-3xl border shadow-lg">
    <div className="min-w-[640px]">
      <div className="calendar-month-weekdays grid grid-cols-7 text-center text-xs font-black uppercase tracking-wider">
      ...
      <div className="grid grid-cols-7">
        {days.map(day => {
          ...
          return <button type="button" key={day.toDateString()} onClick={() => onSelectDay(day)} aria-label={day.toLocaleDateString(language)} className={`calendar-month-day min-h-36 border-t border-r p-3 text-left transition-all ${isOutsideMonth ? 'calendar-month-day-outside' : 'calendar-month-day-current'}`}>
            <div className="flex items-center justify-between">
              <span className={`inline-flex h-8 w-8 items-center justify-center rounded-xl text-xs font-black ${today ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-md shadow-indigo-600/20' : isOutsideMonth ? 'calendar-month-date-outside' : 'calendar-month-date-current'}`}>{day.getDate()}</span>
